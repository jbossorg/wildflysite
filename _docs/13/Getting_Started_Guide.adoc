...
== Getting Started with Wild**Fly** {wildflyVersion}

Wild**Fly** {wildflyVersion} is the latest release in a series of JBoss open-source
application server offerings. Wild**Fly** {wildflyVersion} is an exceptionally fast,
...
Edition {javaee_version} technologies and the technologies available in Wild**Fly** {wildflyVersion}
...
The Wild**Fly** Web Profile doesn't include JMS (provided by HornetQ) by
...
started using Wild**Fly** {wildflyVersion} for your application development. For in-depth
content on administrative features, refer to the Wild**Fly** {wildflyVersion} Admin Guide.
...
can install Wild**Fly** {wildflyVersion} on any operating system that supports the zip or
...
Now that you've downloaded Wild**Fly** {wildflyVersion}, the next thing to discuss is the
...
In " *_standalone_* " mode each Wild**Fly** {wildflyVersion} server instance is an
...
used by the single standalone server run from a Wild**Fly** installation are
...
A key feature of Wild**Fly** {wildflyVersion} is the managing multiple servers from a
...
machines with all Wild**Fly** instances on a given host under the control of
a Host Controller process. The Host Controllers interact with the Domain
Controller to control the lifecycle of the Wild**Fly** instances running on
...
domain mode processes run from a Wild**Fly** installation are found in the
...
[[starting-wildfly-10]]
==== Starting Wild**Fly** {wildflyVersion}

To start Wild**Fly** {wildflyVersion} using the default web profile configuration in "
...
==== Starting Wild**Fly** with an Alternate Configuration
...
As with previous Wild**Fly** releases, you can point your browser to
...
From here you can access links to the Wild**Fly** community documentation
...
Administration Console. Or, if you uncover a defect while using Wild**Fly**,
...
Wild**Fly** {wildflyVersion} but can easily be replaced with your own if desired.

[[managing-your-wildfly-10]]
==== Managing your Wild**Fly** {wildflyVersion}

Wild**Fly** {wildflyVersion} offers two administrative mechanisms for managing your
running instance:
...
By default Wild**Fly** {wildflyVersion} is now distributed with security enabled for the
...
When running locally to the Wild**Fly** process the CLI will silently
